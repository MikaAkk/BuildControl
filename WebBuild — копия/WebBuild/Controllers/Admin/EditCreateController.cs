...
public class EditCreateController : AdminController
{
    private readonly AppDbContext _db;
    private readonly AuditLogService _audit;

    public EditCreateController(AuthService auth, AppDbContext db, AuditLogService audit) : base(auth, db)
    {
        _db = db;
        _audit = audit;
    }

    public  async Task<IActionResult> EditIndex(int? statusFilter, int? positionId, int? roleId)
    {
        IQueryable<Employee> query = _db.Employees
            .IgnoreQueryFilters()
            .Include(e => e.EmployeeStat)
            .Include(a => a.PersonData).ThenInclude(a => a.PhoneNumber)
        ...
        var employees = await query
            ...
            .ThenBy(e => e.PersonData.Surname)
            .ToListAsync();

        ViewBag.Positions = _db.Positions.ToList();
        ViewBag.Roles = _db.Roles.ToList();
        ViewBag.SelectedStatus = statusFilter ?? 0;
        ViewBag.SelectedPosition = positionId ?? 0;
        ViewBag.SelectedRole = roleId ?? 0;

        return View("~/Views/Admin/EmployeeAdmin/EmployeeSelect.cshtml", employees);
    }
    ...
    public IActionResult Create()
    {
        ...
        return View("~/Views/Admin/EmployeeAdmin/CreateEmployee.cshtml");
    }
    ...
    public async Task<IActionResult> Create(EmployeeCreateEditViewModel model)
    {
        if (!ModelState.IsValid)
        {
            ...
            ViewBag.PhoneNumber = _db.PhoneNumbers.ToList();
            ViewBag.States = _db.EmployeeStat.ToList();
            return View("~/Views/Admin/EmployeeAdmin/CreateEmployee.cshtml", model);
        }
        ...
        try
        {
            ...
            await _audit.LogAsync("Create", "Employee", employee.Id,
                $"Создан сотрудник: {person.Surname} {person.Name}, должность ID: {model.PositionId}, роль ID: {model.RoleId}");
    ...
    public async Task<IActionResult> Edit(long id)
    {
        ...
        return View("~/Views/Admin/EmployeeAdmin/EditEmployee.cshtml", viewModel);
    }
    ...
    public async Task<IActionResult> Edit(EmployeeCreateEditViewModel model)
    {
        if (!ModelState.IsValid)
        {
            ...
            ViewBag.Roles = _db.Roles.ToList();
            ViewBag.States = _db.EmployeeStat.ToList();
            return View("~/Views/Admin/EmployeeAdmin/EditEmployee.cshtml", model);
        }
        ...
        try
        {
            ...
            await _audit.LogAsync("Update", "Employee", employee.Id,
                $"Изменён сотрудник: {model.Surname} {model.Name}, должность ID: {model.PositionId}, роль ID: {model.RoleId}");
    ...
    public async Task<IActionResult> Delete(long id)
    {
        ...
        try
        {
            ...
            await _audit.LogAsync("Delete", "Employee", id,
                $"Деактивирован сотрудник: {employee.PersonData?.Surname} {employee.PersonData?.Name}. " +
                $"{reassignedCount} заявок переназначено на менеджера ID {fallbackManagerId}");
    ...
    public async Task<IActionResult> Restore(long id)
    {
        ...
        await _audit.LogAsync("Restore", "Employee", id,
            $"Восстановлен сотрудник: {employee.PersonData?.Surname} {employee.PersonData?.Name}");

        TempData["SuccessMessage"] = $"Сотрудник {employee.PersonData?.FullName} восстановлен.";
        return RedirectToAction(nameof(EditIndex));
    }
